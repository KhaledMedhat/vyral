...
function EmojiPickerEmoji({ emoji, className, ...props }: EmojiPickerListEmojiProps) {
  ...
  return (
    <button
      {...props}
      className={cn("data-active:bg-yellow-700/30 flex size-9 items-center justify-center cursor-pointer rounded-sm text-2xl", className)}
...
function EmojiPickerContent({ className, ...props }: React.ComponentProps<typeof EmojiPickerPrimitive.Viewport>) {
  return (
    <EmojiPickerPrimitive.Viewport
      className={cn(
        "outline-hidden relative flex-1 overflow-y-auto",
        "[&::-webkit-scrollbar]:w-2",
        "[&::-webkit-scrollbar-track]:bg-transparent",
        "[&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/40 hover:[&::-webkit-scrollbar-thumb]:bg-primary/60",
        className
      )}
